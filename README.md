...
#### Video Demo: [Youtube Link](https://youtu.be/Ncr7YsEzzeE)
---
...
* /log_episode (Batch Logging) Allows users to log episode ranges (e.g., "Episodes 1 to 100") via a loop, eliminating the need for manual entry.

  * Current Limitation: The feature currently makes individual API calls for every episode in the loop. Consequently, bulk additions exceeding 200 episodes may trigger API rate limits or timeouts.
  * Future Roadmap: To resolve this, future iterations will implement a static local dataset (JSON or CSV) or an index-based identifier system to bypass the need for repetitive external API calls during bulk operations.
...
* Development: The initial application code is original work. Generative AI (Gemini) was consulted strictly for debugging and CSS formatting assistance.
* Data Source: This project utilizes the One Piece API. (See footer for full attribution).
